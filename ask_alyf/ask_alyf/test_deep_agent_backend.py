...
class UnitTestAskALYFVirtualFilesystem(UnitTestCase):
	...
	def test_source_backend_blocks_path_traversal(self):
		backend = build_ask_alyf_backend()
		with patch("ask_alyf.ask_alyf.tools.ensure_code_search_enabled", lambda: None):
	...
	def test_source_backend_ls_root_handles_disabled_code_search_gracefully(self):
		backend = build_ask_alyf_backend()
		with patch(
			"ask_alyf.ask_alyf.deep_agent_backend.ensure_code_search_enabled",
			side_effect=frappe.ValidationError("Code search is disabled"),
		):
	...
	def test_workspace_scratch_does_not_survive_across_backend_instances(self):
		...
		backend2 = build_ask_alyf_backend()
